...
def _location_terms(location: dict) -> set[str]:
    terms = {_norm(location.get("key")), _norm(location.get("label")), _norm(location.get("state"))}
...
def _contains_alias(text: str, aliases: set[str]) -> str | None:
    return next((alias for alias in aliases if alias and alias in text), None)


def _geo_evidence(post: Post) -> list[dict]:
    fields = [
        ("title", _norm(post.title)),
        ("content", _norm(post.raw_content)),
        ("tags", _norm(post.tags)),
        ("keywords_nlp", _norm(" ".join(str(v) for v in (post.keywords or [])))),
        ("entities_nlp", _norm(" ".join(_flatten_entities(post.entities)))),
    ]
    evidence = []
    for key, config in GEO_KEYWORDS.items():
        aliases = {_norm(key), _norm(config.get("label")), *(_norm(v) for v in config.get("aliases", []))}
        hits = []
        for field, text in fields:
            alias = _contains_alias(text, aliases)
            if alias:
                hits.append({"field": field, "value": alias})
        if not hits:
            continue
        raw_hit = any(hit["field"] in {"title", "content"} for hit in hits)
        nlp_hit = any(hit["field"] in {"keywords_nlp", "entities_nlp"} for hit in hits)
        if raw_hit and nlp_hit:
            confidence = "high"
        elif raw_hit:
            confidence = "high"
        elif nlp_hit:
...
def post_scope_facts(post: Post) -> dict:
    ...
    geo_evidence = _geo_evidence(post)
    locations = []
    territory_values = set()
    for item in geo_evidence:
        location = {key: value for key, value in item.items() if key != "evidence"}
...
def evaluate_post_scope(post: Post, scope) -> dict:
    ...
    keyword_match = True if not keywords else any(keyword in facts["text"] or keyword in facts["keywords"] for keyword in keywords)
    topic_match = True if not topics else bool(facts["topics"].intersection(topics))
    territory_match = True if not territories else bool(facts["territories"].intersection(territories))

    configured = {"source": bool(sources), "keyword": bool(keywords), "topic": bool(topics), "territory": bool(territories)}
    criterion_matches = {"source": source_match, "keyword": keyword_match, "topic": topic_match, "territory": territory_match}
    configured_non_source = [criterion_matches[name] for name in ("keyword", "topic", "territory") if configured[name]]
    ...
    matched_by = [name for name, value in criterion_matches.items() if configured[name] and value]
    return {"matched": matched, "matched_by": matched_by, "match_mode": match_mode, "configured": configured, "criteria": criterion_matches, "facts": facts}
...
def preview_scope(posts: list[Post], scope, limit: int = 8) -> dict:
    ...
    for post in posts:
        ...
        for criterion, value in evaluation["criteria"].items():
            if evaluation["configured"][criterion]:
        ...
        if evaluation["matched"]:
            ...
            unmatched_examples.append({"id": post.id, "title": post.title, "source": post.source, "criteria": evaluation["criteria"], "configured": evaluation["configured"]})
    ...
    for post, evaluation in matched_posts[:limit]:
        examples.append({
            ...
            "geo_evidence": evaluation["facts"]["geo_evidence"],
        })
    ...
    return {
        ...
        "diagnostics": {"source_matches": metric("source"), "keyword_matches": metric("keyword"), "topic_matches": metric("topic"), "territory_matches": metric("territory")},
        "examples": examples,
...
def build_scope_suggestions(posts: list[Post], scope) -> dict:
    topic_counts = Counter(); keyword_counts = Counter(); source_counts = Counter(); territory_counts = Counter()
    for post in posts:
        facts = post_scope_facts(post)
        if facts["source"]: source_counts[post.source or facts["source"]] += 1
        for value in post.topics or []:
            if value: topic_counts[str(value)] += 1
        for value in post.keywords or []:
            if value: keyword_counts[str(value)] += 1
        seen_locations = set()
        for location in facts["locations"]:
            label = location.get("label")
            if label and label not in seen_locations:
                territory_counts[label] += 1; seen_locations.add(label)

    configured_terms = [*(getattr(scope, "keywords", []) or []), *(getattr(scope, "topics", []) or []), *(getattr(scope, "territories", []) or [])]
    configured_norm = {_norm(value) for value in configured_terms if _norm(value)}
    candidate_terms = {*topic_counts.keys(), *keyword_counts.keys(), *territory_counts.keys()}
    ...
    for term in configured_norm:
        for match in get_close_matches(term, list(normalized_lookup.keys()), n=4, cutoff=0.55):
            ...
            if value not in close: close.append(value)
    return {
