...
import { Activity, AlertTriangle, MapPin, Radar, RefreshCw } from "lucide-react";
...
export default function IntelligenceBriefPanel() {
  ...
  const loadBrief = useCallback(async () => {
    ...
    setError("");
    try {
      const data = await getIntelligenceBrief({ windowHours, baselineDays: 30 });
      setBrief(data);
    } catch (requestError) {
      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            type="button"
            onClick={loadBrief}
            className="rounded-lg border border-slate-800 bg-slate-950/40 p-2 text-slate-400 transition hover:text-cyan-300"
            aria-label="Actualizar intelligence brief"
          >
      ...
      {brief && !loading && (
        <>
          <div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-5">
            ...
            <Metric label="Riesgo" value={brief.metrics?.risk_level || "low"} />
          ...
          <div className="mt-5 grid gap-3 lg:grid-cols-3">
            <div className="rounded-xl border border-orange-500/20 bg-orange-500/5 p-4">
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-orange-300">
                <Activity size={15} /> Señal principal
              </div>
              <p className="mt-2 font-medium text-white">
                {brief.leading_signal?.topic || "Sin aceleración relevante"}
              </p>
              <p className="mt-1 text-sm text-slate-400">
                {brief.leading_signal?.growth_score
                  ? `Crecimiento relativo x${brief.leading_signal.growth_score}`
                  : "Sin baseline suficiente para comparar"}
              </p>
            </div>

            <div className="rounded-xl border border-cyan-500/20 bg-cyan-500/5 p-4">
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-cyan-300">
                <MapPin size={15} /> Territorio principal
              </div>
              <p className="mt-2 font-medium text-white">
                {brief.leading_territory?.label || "Sin territorio dominante"}
              </p>
              <p className="mt-1 text-sm text-slate-400">
                {brief.leading_territory
                  ? `${brief.leading_territory.mentions} menciones · ${Math.round((brief.leading_territory.negative_ratio || 0) * 100)}% negativas`
                  : "No se detectaron menciones geográficas en esta ventana"}
              </p>
            </div>

            <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-4">
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-red-300">
                <AlertTriangle size={15} /> Watchlist
              </div>
              <div className="mt-2 space-y-1 text-sm text-slate-300">
                {(brief.watch_items || []).slice(0, 3).map((item, index) => (
                  <p key={`${item.type}-${item.label}-${index}`}>• {item.label}</p>
                ))}
