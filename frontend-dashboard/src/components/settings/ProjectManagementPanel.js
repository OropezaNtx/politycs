  ...
  return (
    <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
      ...
      <form onSubmit={handleSubmit} className="mt-5 grid gap-3 lg:grid-cols-2">
        ...
        <input value={form.sources} onChange={(event) => setForm({ ...form, sources: event.target.value })} placeholder="Fuentes: BBC Mundo, Google News" className="rounded-xl border border-slate-800 bg-slate-950/60 px-4 py-3 text-sm text-white outline-none focus:border-cyan-500/50" />
        ...
        <div className="lg:col-span-2">
          <button type="submit" disabled={saving || !form.name.trim()} className="inline-flex items-center gap-2 rounded-xl bg-cyan-500 px-4 py-3 text-sm font-semibold text-slate-950 transition hover:bg-cyan-400 disabled:cursor-not-allowed disabled:opacity-50">
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2 py-0.5 text-xs text-emerald-300">{project.active ? "Activo" : "Inactivo"}</span>
                </div>
              ...
              <button type="button" onClick={() => handleDelete(project.id)} className="rounded-lg border border-red-500/20 bg-red-500/5 p-2 text-red-300 transition hover:bg-red-500/10" aria-label={`Eliminar ${project.name}`}>
                <Trash2 size={16} />
              </button>
            </div>
          </article>
